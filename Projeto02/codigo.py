...
s_REFRAC = 0.5  # "Fator de courrant" do segundo meio
TRANSICAO = 1  # ponto a partir do qual começa o segundo meio

def calculo(S = s, S_REFRAC = s_REFRAC): #Permite passar outros valores de S
    ...
    E[0] = E0
    E[:, 0] = E_t
    F = np.empty((TIME, LEN+2))  # +2 para comportar condições de contorno
    F[0] = E0
    F[:, 0] = E_t
...
plotPulsos.plot(calculo(S = 1, S_REFRAC = 1)[-1], '--', color='black', label= 'S = 1')
